...
<template>
  <div class="point-position-container">
    <el-row>
      <el-col :span="13">
        <h3>点位分类</h3>
        <e-chart style="height: 136px;width: 100%;" name="classificationChart" ref="classificationChart" type="annular" :chartData="classificationChartData"></e-chart>
      </el-col>
      <el-col :span="11">
        <h3>告警点位</h3>
        <el-row class="alarm-items">
          <template v-for="(item, index) in alarmData">
            <el-col :span="8" :key="index">
              <p>
                <number-grow :value="Number(item.value)" :fontc="item.value > 0 ? '#FF2829' : '#FFFFFF'"></number-grow>
              </p>
              <p>{{item.name}}</p>
            </el-col>
          </template>
    ...
    <div style="height: calc(100% - 188px);">
      <h3>近一周告警</h3>
      <e-chart style="height: calc(100% - 32px);width: 100%;" name="sosChart" ref="sosChart" type="sosLine" :chartData="sosChartData"></e-chart>
    </div>
  </div>
</template>
