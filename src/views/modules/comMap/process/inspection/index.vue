<template>
  <div class='map-check-point-container'>
    <slide-bar position="left" name="巡检列表" v-model="listVisable" :width="425">
      <inspection-list @handleClick="handleClick"></inspection-list>
    </slide-bar>
    <slide-bar position="right" @input="handleClick('init')" :name="resData.id ? '修改卡口' : '新增卡口'" v-model="editVisable">
      <inspection-detail @handleClick="handleClick" :resData="resData"></inspection-detail>
    </slide-bar>
    <slide-bar position="top" v-model="toolVisable">
      <inspection-route @handleClick="handleClick"></inspection-route>
    </slide-bar>
  </div>
</template>
