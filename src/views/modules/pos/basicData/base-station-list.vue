<!--基站管理-->
<template>
  <div class="mod-basestation mod-config">
    ...
    >
      ...
      <el-table-column prop="location" width="140" label="X轴坐标">
        <template slot-scope="scope">
          <span v-if="scope.row.positionX">{{scope.row.positionX }}</span>
          <span v-else>-</span>
        </template>
      </el-table-column>
      <el-table-column prop="location" width="140" label="Y轴坐标">
        <template slot-scope="scope">
          <span v-if="scope.row.positionY">{{scope.row.positionY}}</span>
          <span v-else>-</span>
        </template>
      </el-table-column>
      <el-table-column prop="location" width="140" label="Z轴坐标">
        <template slot-scope="scope">
      ...
      <!-- <el-table-column width="200"
                       label="电池">
        <template slot-scope="scope">
          <el-progress style="padding: 0 10px"
                       v-if="scope.row.rate"
                       :text-inside="true"
                       :stroke-width="15"
                       :status="scope.row.rate>25? 'success' : 'exception'"
                       :percentage="scope.row.rate!==null?Number(scope.row.rate):0"></el-progress>
          <el-tag v-else
                  type="info"
                  effect="dark">未知</el-tag>
        </template>
      </el-table-column>-->
      ...
      <el-table-column width="120" label="操作">
        <template slot-scope="scope">
          <el-button type="text" size="small" @click.stop="addOrUpdateHandles()">
...
<script>
import AddOrUpdate from './base-station-add-or-update'

import { baseStationList, deleteBaseStation } from '@/api/pos/device'
  ...
  methods: {
    ...
    deleteHandle (id, deveui) {
      ...
      this.$confirm(`确定对[设备编码SN=${deveuiArr.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
        ...
        deleteBaseStation(deveuiArr).then(({ data }) => {
          if (data && data.code === 0) {
            this.getDataList()
            this.$message({
              message: '操作成功',
