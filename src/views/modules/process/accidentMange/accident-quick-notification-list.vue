  ...
  <div class="emergency-plan-container mod-config">
    <el-form :inline="true" ref="dataForm" class="el-form-margin-bottom" :model="dataForm" @submit.native.prevent @keyup.enter.native="getDataList()">
      ...
      <el-form-item>
        ...
        <el-button type="warning" size="mini" @click="addOrUpdateHandle()" icon="el-icon-plus" v-waves>新增事故快报</el-button>
    ...
    <el-table :data="dataList" border @cell-click="cellClick" ref="multipleTable" v-loading="dataListLoading" @selection-change="selectionChangeHandle" style="width: 100%;">
      ...
      <el-table-column prop="deptName" label="事故发生单位" width="100" :show-overflow-tooltip="true"></el-table-column>
      <el-table-column prop="occurrenceTime" label="事故发生时间" width="140">
      </el-table-column>
      <el-table-column prop="zoneName" :show-overflow-tooltip="true" label="事故发生地点" width="120">
      </el-table-column>
      <el-table-column prop="accidentType" :show-overflow-tooltip="true" label="事故类别" width="80">
        <template slot-scope="scope">{{scope.row.accidentType | matchTye(accidentTypeList)}}</template>
      </el-table-column>
      <el-table-column prop="casualtySituation" :show-overflow-tooltip="true" label="伤亡情况" min-width="80">
      </el-table-column>
      <el-table-column prop="eventsCourse" :show-overflow-tooltip="true" label="事故经过" min-width="80">
      </el-table-column>
      <el-table-column :show-overflow-tooltip="true" label="事故原因" min-width="80">
        <template slot-scope="scope">
          <span>直接原因：{{scope.row.directCause}}</span>
          <span>间接原因：{{scope.row.indirectCause}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="preventiveMeasures" :show-overflow-tooltip="true" label="防范措施" min-width="80">
        <template slot-scope="scope">
          {{handle(scope.row.preventiveMeasures)}}
        </template>
      </el-table-column>
...
<script>
import { accidentBulletin, accidentBulletinDelete } from '@/api/process/accidentMange'
import waves from '@/utils/waves'
import AddOrUpdate from './accident-quick-notification-add-or-update'
