<template xmlns="">
  <div class='facility-container mod-config'>
    ...
    <el-table :data="dataList" border ref="multipleTable" @cell-click="cellClick" v-loading="dataListLoading" @selection-change="selectionChangeHandle" style="width: 100%;" :cell-class-name="addClass">
      ...
      <el-table-column label="编制人" :show-overflow-tooltip="true">
        <template slot-scope="scope">{{scope.row.createBy.username ? scope.row.createBy.username : '-' }}</template>
      </el-table-column>
      <el-table-column label="编制时间" :show-overflow-tooltip="true">
        <template slot-scope="scope">{{scope.row.createBy.createDate ? scope.row.createBy.createDate : '-' }}</template>

      </el-table-column>
      <el-table-column prop="procedureName" :show-overflow-tooltip="true" label="规程名称">
      </el-table-column>
      <el-table-column prop="content" label="操作规程" :show-overflow-tooltip="true">
        <template slot-scope="scope">
      ...
      <el-table-column label="操作" width="180">
        <template slot-scope="scope">
          <el-button type="text" @click.stop="addOrUpdateHandle(scope.row.id)" :disabled="scope.row.state === '1'">
            修改
          </el-button>
          <el-button type="text" @click.stop="handleSub(scope.row.id)">
            提交
          </el-button>

          <el-button type="text" size="small" :style="{color:scope.row.state === '1' ? '' : '#E63031' }" @click.stop="deleteHandle(scope.row.id,scope.row.sysStationName)" :disabled="scope.row.state === '1'">删除</el-button>

        </template>
      </el-table-column>
    </el-table>
    <el-pagination @size-change="sizeChangeHandle" @current-change="currentChangeHandle" :current-page="pageIndex" :page-sizes="[10, 20, 50, 100]" :page-size="pageSize" :total="totalPage" layout="total, sizes, prev, pager, next, jumper"></el-pagination>
    <operate-instruction-add-or-update v-if="addOrUpdateVisible" ref="addOrUpdate" @refreshDataList="getDataList"></operate-instruction-add-or-update>
  ...
  methods: {
    ...
    handleSub (id) {
      this.$confirm('是否确认提交?', '提示', {
        ...
        let ids = [{ id: id, state: '1' }]
        operateSub(ids).then(({ data }) => {
          if (data && data.code === 0) {
            this.$message({
              message: '提交成功',
              type: 'success'
