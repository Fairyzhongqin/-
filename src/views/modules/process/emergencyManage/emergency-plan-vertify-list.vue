<!--应急预案审核-->
<template>
  <div class="emergency-plan-vertify-container mod-config">
    <el-form :inline="true" ref="dataForm" class="el-form-margin-bottom" :model="dataForm" @submit.native.prevent @keyup.enter.native="getDataList()">
      ...
      <el-form-item>
        ...
        <el-button type="primary" size="mini" @click="submitHandle()" :disabled="dataListSelections.length <= 0" v-waves>
          <icon-svg name="audit" class="tijiao"></icon-svg>批量审核
        </el-button>
      </el-form-item>
    </el-form>
    <el-table :data="dataList" border @cell-click="cellClick" ref="multipleTable" v-loading="dataListLoading" @selection-change="selectionChangeHandle" style="width: 100%;">
      ...
      <el-table-column prop="date" :show-overflow-tooltip="true" label="提交日期" align="center" width="160">
      </el-table-column>
      <el-table-column prop="fileName" label="附件" :show-overflow-tooltip="true">
        <template slot-scope="scope">
      ...
      <el-table-column label="操作" width="100">
        <template slot-scope="scope">
          <el-button type="text" size="small" @click.stop="auditHandle(scope.row.id, scope.row.name)" :disabled="scope.row.state==='1' ?false:true">审核</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <!-- 弹窗, 新增 / 修改 -->
    <vertify-audit v-if="addOrUpdateVisible" ref="addOrUpdate" @refreshDataList="getDataList"></vertify-audit>
  </div>
</template>
<script>
import vertifyAudit from './emergency-plan-vertify-audit'
...
<style lang="scss">
.emergency-plan-vertify-container {
