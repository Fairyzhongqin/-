<template class="safety">
  <el-dialog :title="(!dataForm.id ? '新增' : '修改') + '报告'" :close-on-click-modal="false" append-to-body :visible.sync="visible" width="650px" class="facilty-dialog">
    <div>
      <el-form :model="dataForm" :rules="dataRule" ref="dataForm"  label-width="80px">
        ...
        <el-form-item label="报告信息" class="item-select ueditor_content" >
          <el-select v-model="dataForm.selectVariable" placeholder="请选择">
            <el-option
          ...
          <!--<el-button @click="addUeHtml('label')">添加变量</el-button>-->
          <!--<el-button @click="addUeHtml('time')">添加自动天数</el-button>-->
          <el-popover placement="top" width="160" trigger="hover" v-model="variablesVisible">
            <p>变量不够？想要添加更多! 点这里</p>
            <div style="text-align: right; margin: 0">
              <el-button size="mini" type="text" @click="variablesVisible = false">取消</el-button>
              <el-button type="primary" size="mini" @click="addVariableHandle">确定</el-button>
            </div>
            <i class="el-icon-info iconAdd" slot="reference" @click="variablesVisible = true"></i>
          </el-popover>
          <!--</el-form-item>-->
          <!--<el-form-item prop="ueContent" class="ueditor_content">-->
    ...
    <!--<add-variables v-if="addOrUpdateVisible" ref="addVariables"  @refreshOptios="getReportVariables"></add-variables>-->
  </el-dialog>

</template>

<script>
  import { reportRecordInfo } from '@/api/process/riskJudgment'
  // import addVariables from './production-safety-addVariables'
    ...
    data () {
      return {
        ...
        dataRule: {
          ...
          reportLevel: [
            { required: true, message: '模板等级不能为空', trigger: 'blur' }
          ]
        }
      }
    },
    // components: { addVariables },
    activated () {},
    methods: {
      // 新增单个字段
      init (id) {
        ...
        this.dataForm.msg = ''
        this.dataForm.selectVariable = ''
        this.getReportVariables()
        this.$nextTick(() => {
          if (this.ue) {
            this.ue.setContent('')
          }
            ...
            reportRecordInfo({
              ...
              if (data && data.code === 0) {
                ...
                this.selectedOptions.map(i => {
                  valList.map(j => {
                    if (i.value1 === j) {
                      html = html.replace('$#' + j + '#$', '<span class="addDom"><span class="span1"></span><span class="btn-group">' + i.label + '</span><span>&nbsp;</span></span>')
                    }
                  })
                })
                // 设置编辑器的内容
      ...
      //     }
      //     let tempList = this.selectedOptions
      //     nameList.map((i) => {
      //       tempList.map((j) => {
      //         if (i === j.label) {
      //           html2 = html2.replace('<span class="btn-group">' + i + '</span>', '$#' + j.value1 + '#$')
      //         }
      //       })
      //     })
      ...
      // getReportVariables () {
      //   if (this.dataForm.id) {
      //     findVariablesByTemplateId({ id: this.dataForm.id }).then(({ data }) => {
      //       if (data && data.code === 0) {
      ...
      //             this.dataForm.id = data.id
      //             this.$nextTick(() => {
      //               // this.$refs.addVariables.init(this.dataForm.id)
      //             })
      //           } else {
      //             this.$message.error(data.msg)
      //           }
      ...
      addUeHtml (val) {
        this.newlyAdd = true
        if (this.dataForm.selectVariable) {
          this.$nextTick(() => {
            let addDom = ''
            let option = ''
            this.selectedOptions.map(val => {
              if (this.dataForm.selectVariable === val.value1) {
                option = val.label
              }
            })
            addDom = `<span class="addDom"><span class="span1"></span><span class="btn-group">` + option + `</span><span>&nbsp;</span></span>`
            // 在当前光标位置插入html内容
