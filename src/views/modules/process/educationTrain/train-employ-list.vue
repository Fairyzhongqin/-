<!--从业人员培训-->
<template>
  <div class="train-employy-style mod-config">
    <el-form :inline="true" ref="dataForm" class="el-form-margin-bottom" :model="dataForm" @submit.native.prevent @keyup.enter.native="getDataList()">
      ...
      <el-form-item>
        ...
        <el-button type="warning" size="mini" @click="addOrUpdateHandle('cy')" icon="el-icon-plus" v-waves>上传资料</el-button>
      ...
      <el-table :data="dataList" border @cell-click="cellClick" ref="multipleTable" v-loading="dataListLoading" @selection-change="selectionChangeHandle" style="min-width: 100%;">
        ...
        <el-table-column label="操作" width="180">
          <template slot-scope="scope">
            ...
            <el-button type="text" size="small" @click.stop="addOrUpdateHandle('cy',scope.row.id)">修改</el-button>
...
export default {
  ...
  methods: {
    ...
    // // 单独查询父级类别
    // getParentTrain () {
    //   traintypeList({ trainType: '1', trainCategoryName: this.trainCategoryName }).then(({ data }) => {
    //     if (data && data.code === 0) {
    ...
    selectTrainType () {
      this.trainData = []
      traintypeList({ trainType: '1' }).then(({ data }) => {
        if (data && data.code === 0) {
          if (data.page.list.length > 0) {
