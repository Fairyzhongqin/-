<template>
  <el-dialog class="select-staff" width="750px" title="选择培训人员" :visible.sync="innerVisible" append-to-body>
    <!-- 所选人员 -->
    <el-form ref="form" label-width="60px">
      <el-form-item label="所选人员">
        <div class="personAdd">
          <div class="personAddover">
            <el-tag @close="handleClose(tag)" size="mini" v-for="tag in dataListSelections" :key="tag.gh" closable>{{tag.name}}</el-tag>
          </div>
        </div>
      </el-form-item>
    </el-form>
    <el-form :inline="true" ref="dataForm" :model="dataForm" @keyup.enter.native="getDataList()">
      <el-row>
        ...
        <el-col :span="9">
          <el-form-item label="人员类型" size="mini" prop="cgformId">
            <el-select v-model="dataForm.cgformId" clearable placeholder="请选择">
              <el-option label="全部" value></el-option>
              <el-option v-for="item in cgformIdList" :key="item.cgformId" :label="item.cgformName" :value="item.cgformId"></el-option>
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span="6">
...
export default {
  ...
  methods: {
      ...
      if (this.isRadio) { // 单选
        if (val.length) {
          if (this.dataListSelections.length) {
            if (this.dataListSelections[0].gh !== val[0].gh) {
              this.dataListSelections = []
              this.dataListSelections.push(val[0])
            }
          } else {
            this.dataListSelections.push(val[0])
          }
        ...
        val.map(value => {
          let flag = false
          this.dataListSelections.map(value1 => {
    ...
    addStaff () {
      ...
      this.$emit('selectStaffEmit', this.dataListSelections)
      this.dataListSelections = []
    },
    // 请求选中标签
