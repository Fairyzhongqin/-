<!--岗位资质要求-->
<template>
  <div class="position-q mod-config">
      ...
      <div class="info staff-info">
        <div class="info-content">
          <el-tabs v-model="activeName" @tab-click="handleClick">
            ...
            <el-tab-pane label="培训经历" name="2">培训经历</el-tab-pane>
          </el-tabs>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    renderContent (h, data) {
      return data.stationName || data.deptName
    },
    onExpand (e, data) {
      if ('expand' in data) {
    ...
    toggleExpand (data, val) {
      ...
      if (Array.isArray(data)) {
        data.forEach(function (item) {
          ...
          if (item.child && item.sysStationList && item.sysStationList.length > 0) {
            item.sysStationList.map(value => {
