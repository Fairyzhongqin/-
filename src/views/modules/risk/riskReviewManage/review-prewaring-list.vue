...
<template>
  <div class="control-list-style mod-config">
    ...
    <el-table :data="dataList" border ref="multipleTable" tooltip-effect="dark" @cell-click="cellClick" v-loading="dataListLoading" class="lecTable" @selection-change="selectionChangeHandle" style="width: 100%;">
      ...
      <el-table-column prop="rangeType" min-width="120" :show-overflow-tooltip="true" label="风险点类型">
        <template slot-scope="scope">{{scope.row.rangeType | matchType(rangeTypeList)}}</template>
      </el-table-column>
      <el-table-column prop="zoneName" min-width="110" :show-overflow-tooltip="true" label="所属区域">
      </el-table-column>
      ...
      <el-table-column label="管控措施" header-align="center" width="80" align="center">
        <template slot-scope="scope">
              ...
              <tr>
                ...
                <td class="second-cell">
                  <template v-if="scope.row.gloryMeasures">
                    <el-input type="textarea" v-model="scope.row.gloryMeasures" readonly :autosize="{ minRows: 1, maxRows: 3}">
                    </el-input>
                  </template>
                  <template v-else>
      ...
      <el-table-column prop="nextEvaluateDate" width="140" label="复评日期">
      </el-table-column>
    ...
    <!-- 弹窗, 管控操作 -->
    <indentification-operation v-if="addOrUpdateVisible" ref="addOrUpdate" @refreshDataList="getDataList">
    </indentification-operation>
  </div>
</template>
<script>
import { queryZoneInfo } from '@/api/pos/region'
import { facilitiesList } from '@/api/risk/analysis'
import indentificationOperation from '../riskIdentificationControl/evaluation-identificate-operation'
