<!--风险辨识清单 - 全景地图table-->
<template>
  <el-dialog :title="'风险辨识清单-'+queryConditions.businessName" class="identification-map" :close-on-click-modal="false" :visible.sync="visible" append-to-body width="900px">
    <div>
      <el-table :data="dataList" border ref="multipleTable" height="450" tooltip-effect="dark" @cell-click="cellClick" v-loading="dataListLoading" class="lecTable" @selection-change="selectionChangeHandle" style="width: 100%;">
        <el-table-column type="index" width="50" label="序号"></el-table-column>
        <el-table-column prop="businessName" :show-overflow-tooltip="true" label="风险点名称"></el-table-column>
        <el-table-column prop="rangeType" :show-overflow-tooltip="true" label="风险点类型">
          <template slot-scope="scope">{{scope.row.rangeType | matchType(rangeTypeList)}}</template>
        </el-table-column>
        <el-table-column :show-overflow-tooltip="true" min-width="100" label="可能发生的风险类型">
          <template slot-scope="scope">
            <span>{{handle(scope.row.factorList)}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="mainFactor" min-width="90" :show-overflow-tooltip="true" label="主要风险危害因素">
        ...
        <el-table-column label="辨识日期" prop="evaluateDate" width="145">
        </el-table-column>
        <el-table-column label="辨识人" :show-overflow-tooltip="true" prop="appraiser"></el-table-column>
