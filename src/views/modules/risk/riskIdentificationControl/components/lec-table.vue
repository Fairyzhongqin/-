<template>
  <div class="mod-config">
    <el-table :data="dataList" border ref="multipleTable" tooltip-effect="dark" @cell-click="cellClick" v-loading="dataListLoading" class="lecTable" @selection-change="selectionChangeHandle" style="width: 100%;">
      ...
      <el-table-column prop="businessName" width="100" :show-overflow-tooltip="true" label="风险点名称">
      </el-table-column>
      ...
      <el-table-column label="LEC风险评价" header-align="center">
        <el-table-column header-align="center" label="L" width="45" align="center">
          <template slot-scope="scope">
        ...
        <el-table-column header-align="center" label="C" width="45" align="center">
          <template slot-scope="scope">
            {{scope.row.content ? Math.ceil(scope.row.content.split('/')[2]): '暂无'}}</template>
        </el-table-column>
        <el-table-column header-align="center" label="D" width="45" align="center">
          <template slot-scope="scope">
    ...
    <!-- 弹窗, 历史查看 -->
    <lec-history-view v-if="addOrUpdateVisible" ref="addOrUpdate" @refreshDataList="getDataList">
    </lec-history-view>
  </div>
</template>
<script>
import { getLabels } from '@/api/common/sys'
import lecHistoryView from './lec-history-view'
