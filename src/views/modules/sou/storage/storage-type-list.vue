<template >
  <div class='highrisk-type mod-config'>
    ...
    <el-table :data="dataList"
      ...
      <el-table-column prop="monitoringParam"
                       :show-overflow-tooltip="true"
                       label="监测预警参数">
      </el-table-column>
...
<script>
import addOrUpdate from './storage-type-add-or-update'
import waves from '@/utils/waves'
import { storageFaciltyTypeList, storageTypeDelete } from '@/api/risk/dangerStorageManage'
...
export default {
  ...
  methods: {
    ...
    deleteHandle (id, name) {
      ...
      this.$confirm(`确定对[高危工艺类型名称为=${nameArr.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      })
        .then(() => {
          storageTypeDelete(ids)
            .then(({ data }) => {
              if (data && data.code === 0) {
                console.log(data)
                this.getDataList()
